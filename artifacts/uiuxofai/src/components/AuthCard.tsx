...
import { mockSignInEmail, mockSignInGoogle, type AuthUser } from "../lib/auth";
...
export function AuthCard({ variant = "compact", onSuccess, intent }: AuthCardProps) {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    // For demo: brief "sent" state, then auto-complete after another beat.
    timersRef.current.push(
      window.setTimeout(() => {
        if (mountedRef.current) setStep("sent");
      }, 400),
    );
    try {
      const user = await mockSignInEmail(trimmed);
      if (!mountedRef.current) return;
      timersRef.current.push(
        window.setTimeout(() => {
          if (mountedRef.current) onSuccess(user);
        }, 800),
      );
    } catch {
      if (!mountedRef.current) return;
  ...
  return (
    <div className="w-full">
      ...
      <div className={variant === "full" ? "text-center" : "text-left"}>
        ...
        {step === "sent" && (
          <p className="mt-2 text-[13.5px] leading-[1.55]" style={{ color: SUB }}>
            We sent a sign-in link to <span style={{ color: INK }}>{email}</span>. For this demo,

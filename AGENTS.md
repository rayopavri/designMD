...
- **Custom Firebase auth domain needs two rewrites, not one** — `next.config.ts` must proxy both `/__/auth/:path*` and `/__/firebase/:path*` to `designmd-2ff95.firebaseapp.com`. The OAuth handler page (served via the `/__/auth/*` rewrite) itself fetches `/__/firebase/init.json` to bootstrap its own `firebase.initializeApp()`; if that second rewrite is missing, the fetch 404s against our own app and `signInWithRedirect` silently strands — browser lands back on the app signed out, no error, no `/api/auth/session` call. If Google sign-in ever regresses this way, check Network for that 404 first. Note the reserved URLs also depend on the Firebase Hosting site having *content deployed*: a Hosting site that exists but has zero releases returns Firebase's "Site Not Found" for `/__/firebase/init.json` while `/__/auth/handler` still 200s (it is served by the Auth backend, not Hosting). This bit us on 2026-08-18 and was fixed by deploying a one-file placeholder site (`firebase deploy --only hosting --project designmd-2ff95`). Health check: `curl -s -o /dev/null -w "%{http_code}" https://uiuxskills.com/__/firebase/init.json` must return 200.
- **firebase-admin must stay `require()`-loadable on Vercel** — Next loads `firebase-admin` as an external (CJS `require()`) module. `firebase-admin@14` pulls `jwks-rsa@4` → `jose@^6`, and `jose@6` is ESM-only, so `/api/auth/session` and `/api/me` crashed at runtime with `ERR_REQUIRE_ESM`: every sign-in completed at Google but the session-cookie exchange 500'd, so all users appeared logged out (2026-08-18 incident, fixed in `11eb228`). Local dev does NOT reproduce this — modern local Node (≥22.12) supports `require(esm)`, Vercel's runtime path does not. `package.json` therefore pins `pnpm.overrides` `"jwks-rsa>jose": "^5.10.0"` (jose@5 is dual CJS/ESM; jwks-rsa only uses `importJWK`/`exportSPKI`, API-stable across v5/v6). Do not remove that override while `jwks-rsa` still `require()`s `jose`. After any dependency upgrade touching server-side packages, check production Runtime Logs/Errors (`get_runtime_errors`) for module-load failures — they will not show up locally.

/**
 * bundleListAdapter — converts an API/DB bundle *list* row into the UI's
 * BundleItem shape.
 *
 * Extracted from the (client-only) useBundleItems hook so it can be shared
 * with Server Components: the home and library pages now convert bundles on
 * the server and pass them to the client hooks as `initialItems`, which lets
 * the first HTML payload contain the full list of bundle links (crawlable by
 * Google + AI crawlers that don't execute JavaScript).
 *
 * This module intentionally has NO 'use client' directive: importing a value
 * (function/type) from a 'use client' module into a Server Component turns it
 * into a client-reference proxy that cannot be called on the server, so the
 * converter has to live in a neutral module.
 */
import {
  type BundleItem,
...
export interface ApiBundleListItem {
  id: string;
  slug: string;
  title: string;
  description: string;
...
export interface DbBundleListItem extends Omit<ApiBundleListItem, 'publishedAt' | 'updatedAt'> {
  publishedAt: Date | null;
  updatedAt: Date;
}

/**
 * Serialise a DB list row into the JSON shape the client hook consumes, so
 * server- and client-produced BundleItems are byte-identical (no hydration
 * mismatch). Mirrors what `NextResponse.json()` does for the /api/bundles route.
 */
export function serializeListItem(row: DbBundleListItem): ApiBundleListItem {
  return {
    ...row,
...
export function apiToBundleItem(row: ApiBundleListItem): BundleItem {
